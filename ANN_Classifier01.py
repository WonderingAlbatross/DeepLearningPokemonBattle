...
def start_learning(train_path,test_path,lr,l1_size,l2_size,epoch_num):
    ...
    dataloader = DataLoader(train_tensor, batch_size=1024, shuffle=True)
    ...
    if os.path.exists(save_model_name+'_loss.csv'):
        ...
        print(loss_df.iloc[-1])


    print('se',starting_epoch)
    ...
    if os.path.exists(save_model_name+'_loss.csv'):
        loss_df = pd.read_csv(save_model_name+'_loss.csv')
        pd.concat([loss_df,new_loss_df], sort=False).to_csv(save_model_name+'_train.csv', index=False)
    else:
...
start_learning(train_path,test_path,lr,l1_size,l2_size,epoch_num)